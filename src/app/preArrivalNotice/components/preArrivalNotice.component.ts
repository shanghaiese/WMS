...
import { PreArrivalNoticeService } from '../service/preArrivalNoticeService'
import { RowData } from '../mockData/rowData'
import { OrderListService } from '../../orderList/service/orderList.service'
...
@Component({
  ...
  template:`<div style="width: 200px; height: 100%; top:-3px !important;position: relative">
            ...
            <modal #modal [keyboard]="false" [backdrop]="'static'" (onClose)="closed()" (onDismiss)="dismissed()" (onOpen)="opened()" cssClass="modal-xl">
                ...
                <modal-footer>
                      <button class="btn btn-primary" (click)="closed()">取消</button>
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////主组件/////////////////////////////////////////////////////////////////////////////////////////////////////////////
@Component({
  templateUrl: './app/preArrivalNotice/components/preArrivalNotice.html'
})
export class PreArrivalNoticeComponent{
  ...
  constructor(private preArrivalNoticeService:PreArrivalNoticeService) {
  ...
  private getRowData(){
    this.preArrivalNoticeService.getAsnList('asn')
        .then(resp => {
          var arr = resp.obj.records;
          console.log("po-list="+JSON.stringify(arr.length));
          for(var i=0;i<arr.length;i++){
            arr[i].dock = arr[i].dock.dockCode +'-'+ arr[i].dock.dockUuid;
            arr[i].whoCreate = arr[i].whoCreate.name +'-'+ arr[i].whoCreate.time +'-' + arr[i].whoCreate.uuid;
            arr[i].whoEdit = arr[i].whoEdit.name + '-' + arr[i].whoEdit.time + '-' + arr[i].whoEdit.uuid;
            arr[i].owner = arr[i].owner.ownerCode + '-' + arr[i].owner.ownerNameC+ '-' + arr[i].owner.ownerNameE+ '-' + arr[i].owner.ownerUuid;
            arr[i].supplier = arr[i].supplier.supplierCode + '-' + arr[i].supplier. supplierNameC+ '-' + arr[i].supplier.supplierNameE+ '-' + arr[i].supplier.supplierUuid;
   ...
   */
  checkOrder():void{
    this.preArrivalNoticeService.checkOrders('asn','1234','audit')  //po/:uuid/audit
        .then(resp => {
   ...
   */
  closeOrder():void{
    this.preArrivalNoticeService.closeOrders('asn','1234','close')   //po/:uuid/close
        .then(resp => {
   ...
   */
  deleteOrder():void{
    this.preArrivalNoticeService.deleteOrders('asn','1234')   //po/:uuid
        .then(resp => {
